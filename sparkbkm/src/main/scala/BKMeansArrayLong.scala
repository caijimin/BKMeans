/*
 * BKMeansArrayLong for compact binary code
 *
 */
 ...
 */
class BKMeansArrayLong private (
    ...
    @native def nativeNearestCenter(centers: Array[Array[Long]], point: Array[Long]) : Int
      ...
      val totalContribs = data.mapPartitions { points =>
        ...
        points.foreach { point =>
          (0 until runs).foreach { i =>
            val (bestCenter, cost) = BKMeansArrayLong.findClosest(thisActiveCenters(i), point)
...
//println("-------- new centers: [" + centers(run).mkString(", ") + "]")
    ...
    if (iteration == maxIterations) {
      println("KMeans reached the max number of iterations: " + maxIterations)
    } else {
      println("KMeans converged in " + iteration + " iterations.")
    }
...
object BKMeansArrayLong {
  ...
  def predict(centers: Array[Array[Long]], point: Array[Long]) : Int = {
    BKMeansArrayLong.findClosest(centers, point)._1
  }
